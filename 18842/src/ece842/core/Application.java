...
//		}
		...
		while (true) {
			if (task.equalsIgnoreCase("a")) {
				...
				if (choice.equalsIgnoreCase("y")) {
					Message msg = new TimeStampedMessage("logger", "log", id.concat(" Self-Event"));
					msg.setSource(id);
	...
	private static void handleSendRequest(Scanner scanIn,
		...
		try {
			...
			if (choice.equalsIgnoreCase("y")) {
				...
				Message msg = new TimeStampedMessage("logger", "log", message.toString());
				msg.setSource(id);
	...
	private static void handleReceiveRequest(MessagePasser messagePasser) {
		...
		try {
			...
			if (message != null) {
				System.out.println(String.format("Message received from %s. with seqNo:%s Request Type:%s, Duplicate Flag is %s, Data is as follows:%s\n",
						message.getSource(),
